...
class MetaLambdaBuilder(type):
    ...
    def __init__(self, *args, **kw):
        ...
        sage_funcs = [
            x for x in list(globals().keys()) if "sage.functions" in str(type(eval(x)))
        ]
...
class Magic(object, metaclass=MetaLambdaBuilder):
        ...
        def __call__(self, *args):
            try:
                ...
                if len(args) == self.order:
                    ...
                    raise TypeError(msg.format(self.order, self.order + 1, len(args)))
            except TypeError:
    ...
    def __init__(self):
        ...
        for name, obj in list(scipy.stats.__dict__.items()):
            if not hasattr(obj, "cdf"):
                continue
            if not hasattr(obj, "_parse_args"):
                continue
    ...
    def argParse(arg, *args):
        ...
        types = {
            "n": [int, float, Rational, Integer, sage.rings.real_mpfr.RealLiteral],
            "f": [
                float,
                Rational,
    ...
    def forms(self, x, **kwargs):
        ...
        def try_sympy_factory(sympy_name, **kwargs):
            def try_sympy(x):
                ...
                if not sympy_contains_decimal(x._sympy_()) and sympy_contains_decimal(
                    result._sympy_()
                ):
                    return []
                return [result]
        ...
        if not safe:
            for name in ["pow", "powdenest", "pow_base", "exp_log", "log"]:
                ...
                simplifiers["py_%s_f" % name] = try_sympy_factory(
                    simplifier, force=True
                )
                ...
                for name, simplifier in list(simplifiers.items()):
                    ...
                    try:
                        for new_form in simplifier(form):
                            ...
                            if not (
                                new_form in forms
                                or new_form in old_forms
                                or new_form in new_forms
                            ):
    ...
    def sub(self, exp, subs):
        ...
        for i in x:
            if i.is_relational():
                temp = i
                for s in subs:
                    temp = self.sb(temp.lhs(), s.lhs(), s.rhs()) == self.sb(
                        temp.rhs(), s.lhs(), s.rhs()
                    )
                out.append(temp)
            else:
                temp = i
    ...
    def __call__(self, *args, **kwargs):
        if self.argParse("f", *args):
            ...
            return self.unravel(
                [m.apply_map(lambda x: S.ss(x, **kwargs)) for m in args]
            )
